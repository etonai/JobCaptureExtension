...
- every "Open Job Search" / "Open Premium Job Search" press appends a search-tracking.csv row with postsSeen = 25, recentPostings = 0, the configured freshness label, exactMatches = UNKNOWN, and a blank notes column; older two-, three-, five-, and six-column files migrate in place, with historical exact-match values backfilled to UNKNOWN and notes backfilled blank
...
`search-tracking.csv` includes a trailing `notes` column for the user's own free-text notes about a search — the extension never reads, validates, or interprets it. New rows and rows migrated from older schemas start with `notes` blank. Every write path that rewrites an existing row (Refresh, Next Page, and schema migration) carries whatever text is already in that row's `notes` field through unchanged, so hand-typed notes (including commas or multi-line text, which round-trip through normal CSV quoting) survive later tracking updates.
